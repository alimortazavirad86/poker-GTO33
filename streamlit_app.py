...
def gto_preflop_mix(hand:str, position:Position, stack_bb:float, players_left:int, total_players:int,
                    facing_open:bool=False, open_size_bb:float=2.5, facing_shove:bool=False, shove_size_bb:float=0.0, raise_level:int=1,
    ...
    # Facing an open
    if facing_open:
        # Adjust thresholds if this is a 3-bet or 4-bet spot
        lvl = max(1, int(raise_level))
        val, blf, call = _preflop_three_bet_split(hi,lo,suited,position,open_size_bb,stack_bb)
        if lvl>=2:
            # Tighter continue ranges vs 3-bets / 4-bets
            val *= 1.20
            blf *= 0.40 if lvl==2 else 0.25
            call *= 0.70 if lvl==2 else 0.45
        if strict:
            exploit_adj = 0.0
        # Exploit adjustments
        ...
        label_val = '3B-VALUE' if lvl==1 else ('4B-VALUE' if lvl==2 else '5B-VALUE')
        label_blf = '3B-BLUFF' if lvl==1 else ('4B-BLUFF' if lvl==2 else '5B-BLUFF')
        add(label_val, val*(1-jam_share)); add('JAM-VALUE', val*jam_share)
        add(label_blf, blf*(1-jam_share)); add('JAM-BLUFF', blf*jam_share)
        add('CALL', call)
        ...
        return {
            ...
            'mix': mix, 'all': allp,
            'explain': f"Stack {stack_bb:.0f}bb; lvl={lvl} vs size {open_size_bb:.1f}; exploit {exploit_adj:+.2f}"
        }
        val, blf, call = _preflop_three_bet_split(hi,lo,suited,position,open_size_bb,stack_bb)
        # Exploit adjustments
        if not strict:
...
def simulate_up_to_hero(street_step:int, btn:int, seats:int, actions:Dict[int,Dict[str,float]], hero:int) -> Tuple[Dict[int,float], float, float, Optional[int]]:
    ...
    for s in order:
        ...
        pot_total = sum(contribs.values())
    return contribs, current_bet, pot_total, last_agg
def analyze_raises_after_hero(street_step:int, btn:int, seats:int, actions:Dict[int,Dict[str,float]], hero:int) -> Dict[str, int]:
    """Return how many distinct bet-increases (raises) happened BEFORE hero and AFTER hero."
    We count transitions where a Bet/Raise sets a target strictly higher than current_bet.
    """
    contribs = {i:0.0 for i in range(seats)}
    if street_step==0:
        sb = (btn + 1) % seats; bb = (btn + 2) % seats
    ...
    for s in order:
        ...
        if t=="Bet/Raise":
            ...
            need = max(0.0, current_bet - prev)
            contribs[s] = prev + need
        # Folds don't change current_bet
    return {"before": raises_before, "after": raises_after}
...
if result:
    ...
    # === Automatic behind-action re-evaluation (preflop only) ===
    if street=='preflop':
        ra = analyze_raises_after_hero(step, btn_seat, SEATS, cur_actions, hero_seat)
        if ra.get('after',0) > 0 and current_bet_full > current_bet_now + 1e-9:
            # Facing further action behind; compute new facing amount
            facing_again = max(0.0, current_bet_full - hero_prev)
            level = 1 + ra.get('before',0) + ra.get('after',0)  # 1=open,2=3bet,3=4bet
            res2 = gto_preflop_mix(
                hand=hero_hand, position=(hero_pos_label if hero_pos_label in ['UTG','MP','CO','BTN','SB','BB'] else 'MP'),
                stack_bb=float(stack_bb), players_left=int(players_left), total_players=int(total_players),
                facing_open=True, open_size_bb=float(current_bet_full), facing_shove=False, shove_size_bb=0.0,
                pot_before_hero_bb=float(pot_full), exploit_adj=exploit_adv, raise_level=level
            )
            st.info(f"Action behind increased the price to **{current_bet_full:.1f} BB** (level {level}). New decision:")
            pct2 = int(round(_clamp(res2.get('best_pct',0.0),0.0,1.0)*100))
            st.success(f"{res2.get('best','')} — {pct2}%")
            allp2 = res2.get('all', {})
            if allp2:
                st.markdown("**Action mix (after raises behind)**")
                for lbl, p in sorted(allp2.items(), key=lambda kv: kv[1], reverse=True):
                    lab = 'BET/RAISE' if lbl == 'RAISE' else lbl
                    st.write(f"• {lab}: **{int(round(_clamp(p,0,1)*100))}%**")
# ================== Live Table (Plotly) ==================
